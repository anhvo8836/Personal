...
# Install KDE Plasma on the new system and enable the SDDM display manager
    ...
    echo "[Packages] 'xorg-server' 'xorg-xinit' 'plasma-meta' 'sddm' 'plasma-wayland-session' 'egl-wayland' 'print-manager' 'network-manager-applet' 'konsole' 'kate' 'dolphin' 'ark' 'kcalc' 'kdeconnect'"
    echo ""
    pacman -S xorg-server xorg-xinit plasma-meta sddm plasma-wayland-session egl-wayland print-manager network-manager-applet konsole kate dolphin ark kcalc kdeconnect --needed --noconfirm --disable-download-timeout
    echo ""
